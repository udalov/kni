...
// TODO (!): decouple common native code from Objective-C-specific
...
class JVMDeclarationsCache {
    ...
    JVMDeclarationsCache(JNIEnv *env): env(env) {
        objcObjectClass = findClass("kni/objc/ObjCObject");
        objcSelectorClass = findClass("kni/objc/ObjCSelector");
        pointerClass = findClass("kni/objc/Pointer");
        nilClass = findClass("kni/objc/Nil");

        objcObjectPointerField = env->GetFieldID(objcObjectClass, "pointer", "J");
        pointerPeerField = env->GetFieldID(pointerClass, "peer", "J");
        nilInstanceField = env->GetStaticFieldID(nilClass, "INSTANCE", "Lkni/objc/Nil;");
...
class MsgSendInvocation {
    ...
    void *invoke() {
        ...
        if (status != FFI_OK) {
            // TODO: throw a proper JVM exception
...
Type typeFromReflectString(const std::string& name) {
    if (name == "char") return Type(TYPE_CHAR);
    ...
    else if (name == "interface kni.objc.ObjCClass") return Type(TYPE_CLASS);
    else if (name.find("interface kotlin.Function") == 0) {
        // TODO: support returning closures
        fprintf(stderr, "Returning functions from native code is not supported yet\n");
        exit(42);
    } else {
        ...
        if (className == "kni.objc.Pointer") return Type(TYPE_POINTER);
        else if (className == "kni.objc.ObjCSelector") return Type(TYPE_SELECTOR);
        else if (className == "kni.objc.ObjCObject") return Type(TYPE_ID);
        else {
